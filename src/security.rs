...
const ALLOWED_EXTENSIONS: &[&str] = &["txt", "md", "markdown", "rtf", "ps1", "bat", "cmd", "sh", "exe", "py"];
...
pub fn is_execution_allowed(path: &Path) -> bool {
    path.extension()
        .and_then(|e| e.to_str())
        .map(|e| e.to_lowercase())
        .map(|ext| matches!(ext.as_str(), "bat" | "cmd" | "ps1" | "sh" | "exe" | "py"))
        .unwrap_or(false)
}
...
pub fn validate_filename(filename: &str) -> Result<(), String> {
    ...
    let name_upper = filename.split('.').next().unwrap_or("").trim().to_uppercase();
